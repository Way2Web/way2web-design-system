...
export class WayDropdown {
  ...
  @Element() el: HTMLWayDropdownElement;
  ...
  componentDidLoad() {
    this.popover = new Popover(this.trigger, this.positioner, {
      ...
      onAfterHide: () => this.wayAfterHide.emit(),
      onAfterShow: () => this.wayAfterShow.emit(),
      onTransitionEnd: () => {
        if (!this.open) {
  ...
  async hide() {
    ...
    const wayHide = this.wayHide.emit();
    if (wayHide.defaultPrevented) {
      this.open = true;
      return;
    }
  ...
  handleDocumentKeyDown(event: KeyboardEvent) {
    ...
    if (event.key === 'Tab') {
      ...
      if (this.open && document.activeElement?.tagName.toLowerCase() === 'way-menu-item') {
  ...
  handleMenuItemActivate(event: CustomEvent) {
    const item = event.target as HTMLWayMenuItemElement;
  ...
  handlePanelSelect(event: CustomEvent) {
    ...
    if (this.closeOnSelect && target.tagName.toLowerCase() === 'way-menu') {
      this.hide();
      this.focusOnTrigger();
    }
  }
  ...
  handleTriggerKeyDown(event: KeyboardEvent) {
    ...
    const menuItems = menu ? [...menu.querySelectorAll('way-menu-item')] : null;
  ...
  //
  // For example, the accessible trigger of an <way-button> is a <button> located inside its shadow root.
  //
